...
if (Test-Path -Path $netdrive){
   ...
   Write-host "Create Sage 50 Data Copy folder"
   Copy-Item -Path "D:\Sage 50 Data\" -Destination "D:\Sage_50_Data_copy\" -Recurse -Container -Force

   Write-host "Compressing Sage 50 Data folder with 7zip"
   & "C:\Program Files\7-Zip\7z.exe" a -mx=5 "D:\$filename.zip" "D:\Sage_50_Data_copy\*" | Out-File -FilePath $log_folder\Sage_50_Data_zip.txt

   Write-host "Moving the compressed file to the Isilon..."
   Move-Item -Path "D:\$filename.zip" -Destination $sage50data_dst

   Write-host "Removing Sage 50 Data Copy folder..."
   Remove-Item "D:\Sage_50_Data_copy\" -Recurse -Force
   ...
   -from "Thunderbird Sage Daily Backup <printer@atomiccartoons.com>" -to "Atomic Cartoons IT Dept <it@atomiccartoons.com>" `
   -subject "Daily Backup completed" -body "Network drive was online. Backup was completed successfully. <br><br>$body" `
   -BodyAsHTML `
   -Attachments "$log_folder\$filename.txt","$log_folder\Sage_50_Data_zip.txt"
}
else{
